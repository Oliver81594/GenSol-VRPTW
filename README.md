...
* Afterwards I focused on local search to immprove the solutions. I implemented [2-Opt](https://en.wikipedia.org/wiki/2-opt) search and random mutation in Genetic Engine. Again, it may be interestting to note that pure 2-Opt could not be used in isolation as the it may cause violetion of the customers' Time Windows. This was solved by checking rebuilding  the new results into a temporary `Route` object to prove that arriving at subsequent nodes later will not violate any `DueDate` constraints.
